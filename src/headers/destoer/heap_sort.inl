...
inline u32 left(u32 idx) 
{
	return (2 * idx) + 1;
}

inline u32 right(u32 idx)
{
	return (2 * idx) + 2;
}

inline u32 parent(u32 idx)
{
	return (idx - 1) / 2;
}
...
inline void verify(const Array<T>& heap,u32 i, u32 len, F CMP_FUNC)
{
    for(; i < len; i++)
    {
        const u32 r = right(i);
...
template<typename T, typename F>
inline void heapify(Array<T>& heap, u32 idx, u32 len, F CMP_FUNC)
{
...
template<typename T, typename F>
inline void heap_sort(Array<T>& arr,F CMP_FUNC)
{
